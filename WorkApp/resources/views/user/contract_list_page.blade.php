                        ...
                        @foreach ($contrast_list as $info)
                            <div class="card" style="margin-top:20px;">
                                ...
                                <div class="card-body">
                                    ...
                                    @if($info->completed_flg == NUll)
                                    <form action="{{ url('work_complete') }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="work_id" value="{{ $info->id }}">
                                        <input type="hidden" name="order_person_id" value="{{ $info->order_person_id }}">
                                        <button type="submit" class="btn btn-warning">納品完了ボタン</button>
                                    </form>
                                    @else
                                    <a href="#" class="btn btn-light">納品済み</a>
                                    @endif
                                </div>
                            </div>
                        @endforeach
